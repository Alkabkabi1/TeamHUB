<script lang="ts">
    ...
    import DemoRoleSwitcher from '@/components/app/DemoRoleSwitcher.svelte';
    import { t } from '@/lib/i18n.svelte';
    import { themeState } from '@/lib/theme.svelte';
    import type { AppNavItem, SidebarWorkspace } from '@/types/app-dashboard';

    const iconMap: Record<string, IconSvgElement> = {
        home: Home01Icon,
        workspaces: UserGroup03Icon,
    ...
    const app = $derived(
        page.props.app as
            | { nav: AppNavItem[]; workspaces: SidebarWorkspace[] }
            | null
            | undefined,
    );
    const demo = $derived(
...
>
    <div class="border-b p-5" style="border-color: var(--th-border)">
        <div class="flex items-center gap-2.5">
            ...
            <div>
                <p class="font-bold" style="color: var(--th-text)">
                    {t('app.name')}
                </p>
                <p class="text-xs" style="color: var(--th-text-muted)">
                    {t('dashboard.platform_tagline')}
                </p>
            </div>
        </div>
    </div>
