<script lang="ts">
    import type { Project } from '@/components/team-hub/mock-data';
    import {
        ComputerIcon,
        Megaphone01Icon,
        SmartPhone01Icon,
...
<article class="th-card flex flex-col p-5">
    <div class="mb-3 flex items-start justify-between">
        <div
            class="flex size-10 items-center justify-center rounded-xl"
            style="background: color-mix(in srgb, {project.color} 15%, transparent)"
        >
            <HugeiconsIcon icon={icons[project.icon]} size={20} color={project.color} />
        </div>
        <span class="text-sm font-semibold" style="color: {project.color}">{project.progress}%</span>
    </div>

    <h3 class="font-semibold" style="color: var(--th-text)">{project.title}</h3>
    <p class="mt-1 line-clamp-2 text-sm" style="color: var(--th-text-muted)">{project.description}</p>

    <div class="mt-4 h-1.5 overflow-hidden rounded-full" style="background: var(--th-border)">
        <div
