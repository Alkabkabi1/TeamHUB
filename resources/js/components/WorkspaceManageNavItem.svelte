<script lang="ts">
    ...
    // Highlight whenever the current page is any managed workspace dashboard.
    const isActive = $derived(
        managedWorkspaces.some((workspace) =>
            url.isCurrentUrl(manage(workspace.id), url.currentUrl),
        ),
    );
...
{#if managedWorkspaces.length === 1}
    <!-- Manages exactly one workspace: a direct dashboard link. -->
    <Link
        href={toUrl(manage(managedWorkspaces[0].id))}
        onclick={onNavigate}
        class={linkClass}
    >
        <span class={labelClass}>{t('nav.workspace_lead_dashboard')}</span>
        <HugeiconsIcon
            strokeWidth={2}
...
{:else if managedWorkspaces.length > 1}
    <!-- Manages several workspaces: a dropdown to pick which dashboard to open. -->
    <DropdownMenu>
        ...
        <DropdownMenuContent align="start" dir={direction} class="min-w-52">
            {#each managedWorkspaces as workspace (workspace.id)}
                <DropdownMenuItem>
                    {#snippet child({ props })}
                        <Link
                            href={toUrl(manage(workspace.id))}
                            onclick={onNavigate}
                            dir={direction}
