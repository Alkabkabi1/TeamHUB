<script lang="ts">
    ...
    function managerRolesFor(member: WorkspaceManageMember): string[] {
    ...
    let userQuery = $state('');
    let userResults = $state<WorkspaceManageFoundUser[]>([]);
        ...
        searchTimer = setTimeout(async () => {
            ...
            try {
                const result = (await http.submit(
                    searchMembers({ workspace: workspaceId }, { query: { q } }),
                )) as { users: WorkspaceManageFoundUser[] };
    ...
    function removeMember(member: WorkspaceManageMember): void {
    ...
    let roleTarget = $state<WorkspaceManageMember | null>(null);
    let selectedRoles = $state<string[]>([]);

    function openRoleModal(member: WorkspaceManageMember): void {
        roleTarget = member;
