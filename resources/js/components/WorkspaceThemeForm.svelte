<script lang="ts">
    ...
    /**
     * Shared workspace theme editor (color + logo) posting to PUT /workspaces/{id}/theme.
     * Reused by the standalone WorkspaceTheme page (with live preview) and inline in
    ...
    const initialColor = workspace.theme ?? '#c8924a';
    ...
    function submit(e: SubmitEvent): void {
        e.preventDefault();
        form.put(`/workspaces/${workspace.id}/theme`, {
            forceFormData: true,
            preserveScroll: true,
        });
    }
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <div class="flex flex-col gap-2">
            <label
                ...
                class="text-start text-[14px] text-[#7e7e7e]"
            >
                {t('dashboard_workspace_lead.theme_primary_color')}
            </label>
            <div
                ...
                >
                    {#each SWATCHES as swatch (swatch)}
                        <button
                            ...
                            aria-label={t(
                                'dashboard_workspace_lead.pick_color',
                                {
                                    color: swatch,
                                },
                            )}
        ...
        <div class="flex flex-col gap-2">
            <label
                ...
                class="text-start text-[14px] text-[#7e7e7e]"
            >
                {t('dashboard_workspace_lead.upload_logo')}
            </label>
            <div
                class="flex h-10 items-center gap-3 rounded-[10px] border border-black/20 bg-white px-4"
            >
