<script lang="ts">
    ...
    import ManageMembersSection from '@/components/workspaces/manage/ManageMembersSection.svelte';
    import WorkspaceThemeForm from '@/components/WorkspaceThemeForm.svelte';
    import { formatNumber, t } from '@/lib/i18n.svelte';
    import type { WorkspaceBranding } from '@/types';
    import type {
        WorkspaceManageMember,
        WorkspaceManagePendingMember,
        WorkspaceManageRoleOption,
    } from '@/types/workspace-manage';
    ...
    type ManageWorkspace = WorkspaceBranding & { university: string | null };
    ...
    type Props = {
        ...
        roleOptions: WorkspaceManageRoleOption[];
        stats: DashboardStats;
        workspaceStats: WorkspaceStats;
        workspaceProjects: WorkspaceProject[];
        recentActivity: RecentActivity[];
        members: WorkspaceManageMember[];
        pendingApplications: WorkspaceManagePendingMember[];
    };
    ...
    const membersReportUrl = $derived(
        reportMembers.url(workspace.id, { query: { locale: reportLocale } }),
    );

    const statCards: Stat[] = $derived([
        {
            label: t('dashboard_workspace_lead.stats.members'),
            value: formatNumber(stats.membersCount),
            sub: t('dashboard.members_this_month'),
            icon: UserGroupIcon,
        },
        {
            label: t('dashboard_workspace_lead.stats.applications'),
...
<AppHead title={t('dashboard_workspace_lead.title')} />
...
<div class="flex flex-1 flex-col bg-[#fdfdfd]">
    ...
    >
        <WorkspaceManageShell active="overview" {workspace} />

        <section
            aria-label={t('dashboard_workspace_lead.hero_aria')}
            class="w-full"
        >
            <!-- Mobile / tablet hero -->
            <div
            ...
            >
                ...
                >
                    ...
                    >
                        {workspace.university ?? t('workspace.organization')}
                    </p>
                    <p
                        ...
                        {#each workspaceProjects as project (project.id)}
                            ...
                            >
                                <div class="space-y-2">
                                    ...
                                    >
                                        {project.description ??
                                            t('workspace.default_description')}
                                    </p>
                                    <div
        ...
        {#if can(CAP.workspace)}
            <section
                aria-label={t('dashboard_workspace_lead.theme_section')}
                class="flex flex-col gap-5"
            >
                <SectionHeading
                    title={t('dashboard_workspace_lead.theme_customize')}
                />
        ...
        {#if can(CAP.reports)}
            ...
            >
                ...
                <div class="grid grid-cols-1 gap-5 sm:max-w-sm">
                    <ReportCard
                        title={t('dashboard_workspace_lead.report_members')}
                        description={t(
