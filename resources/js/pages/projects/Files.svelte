<script lang="ts">
    ...
    function submit(event: SubmitEvent): void {
        ...
        form.post(`/workspaces/${workspace.id}/projects/${project.id}/files`, {
            forceFormData: true,
            preserveScroll: true,
            onSuccess: () => {
                form.reset();
    ...
    function removeFile(id: number): void {
        router.delete(
            `/workspaces/${workspace.id}/projects/${project.id}/files/${id}`,
            {
                preserveScroll: true,
            },
        );
...
<AppHead title={`${project.name} — ${t('app.files')}`} />
...
<div class="flex flex-1 flex-col bg-[#fdfdfd]">
    ...
    >
        <ProjectManageShell active="files" {workspace} {project} />
