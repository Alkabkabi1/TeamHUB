<script lang="ts">
    ...
    import type { ProjectRef, WorkspaceRef } from '@/types';
    ...
    function submit(event: SubmitEvent): void {
        ...
        form.post(`/workspaces/${workspace.id}/projects/${project.id}/tasks`, {
            preserveScroll: true,
            onSuccess: () => {
                form.reset();
    ...
    function detailUrl(taskId: number): string {
        return `/workspaces/${workspace.id}/projects/${project.id}/tasks/${taskId}`;
    }
...
<AppHead title={`${project.name} — ${t('tasks.title')}`} />
...
<div class="flex flex-1 flex-col bg-[#fdfdfd] dark:bg-[#0f172a]">
    ...
    >
        <ProjectManageShell active="tasks" {workspace} {project} />
