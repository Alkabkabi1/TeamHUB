...
<AppHead title="{t('join.title')} — {workspace.name}" />

<AppAuthLayout
    title="{t('join.title')} — {workspace.name}"
    backHref={show(workspace.id).url}
    wide
>
    <Form {...store.form(workspace.id)} class="flex w-full flex-col">
        {#snippet children({ errors, processing })}
            <InputError message={errors.workspace} class="mb-4 text-center" />
