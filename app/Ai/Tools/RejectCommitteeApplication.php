 ...
 */
class RejectCommitteeApplication extends WriteTool
{
    public function description(): Stringable|string
    {
        return 'Reject a pending committee join request. Requires ManageMembers capability.';
    }
    ...
    public function schema(JsonSchema $schema): array
    {
        return [
            'membership_id' => $schema->integer()
                ->description('The CommitteeMembership ID to reject.')
                ->required(),
    ...
    protected function preview(Request $request): array
    {
        ...
        return [
            'summary' => "رفض طلب {$membership->user->name} في لجنة {$membership->committee->name}",
            'changes' => [
                "رفض طلب {$membership->user->name} في لجنة {$membership->committee->name}",
            ],
            'params' => [
