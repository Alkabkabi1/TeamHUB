 ...
 */
function projectMembership(User $user, Project $project, array $roles): ProjectMembership
{
...
test('a project lead has the full capability set, a member has none', function () {
    ...
    projectMembership($lead, $project, [ProjectRole::ProjectLead]);

    $member = User::factory()->student()->create();
    projectMembership($member, $project, [ProjectRole::Member]);
...
test('managedProjects returns only projects where the user holds a manager role', function () {
    ...
    projectMembership($user, $led, [ProjectRole::ProjectLead]);
    projectMembership($user, $plain, [ProjectRole::Member]);
...
test('project capability gates resolve through the user methods', function () {
    ...
    projectMembership($membershipManager, $project, [ProjectRole::MembershipManager]);
